...
async function _getSNXBalances({ providerUrl }) {
	...
	for (let i = 0; i < addresses.length; i++) {
		...
		console.log(
			gray(`  > ${i}/${addresses.length} - ${address}: ${ethers.utils.formatEther(balance)} SNX`)
		);
...
async function _getUSDBalances({ providerUrl }) {
	...
	for (let i = 0; i < addresses.length; i++) {
		...
		console.log(
			gray(`  > ${i}/${addresses.length} - ${address}: ${ethers.utils.formatEther(balance)} sUSD`)
		);
...
async function _getAllAddressesThatDeposited({ providerUrl }) {
	...
	console.log(
		gray(`* Retrieving all SynthetixBridgeToBase MintedSecondary events in ${providerUrl}...`)
	);
	...
	for (const event of events) {
		...
		const entry = _data.accounts[account] || {
			totalDeposited: '0',
			numDeposits: '0',
		};

		// Update entry
		entry.totalDeposited = ethers.BigNumber.from(entry.totalDeposited)
			.add(amount)
			.toString();
		entry.numDeposits = ethers.BigNumber.from(entry.numDeposits)
