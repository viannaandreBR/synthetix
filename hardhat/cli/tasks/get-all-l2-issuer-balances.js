...
async function getIssuerBalances({ providerUrl, dataFile, network, minimumBalance, useOvm }) {
	...
	for (let i = 0; i < issuers.length; i++) {
		...
		console.log(
			gray(
				`  > Issuer ${i + 1}/${issuers.length} - ${account}: ${ethers.utils.formatEther(
					balance
				)} sUSD > Total: ${ethers.utils.formatEther(total)} sUSD`
			)
		);
