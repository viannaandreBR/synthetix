...
const { logSection, logItem } = require('../utils/prettyLog');
...
async function rates({ network, useOvm, providerUrl }) {
	...
	logItem(
		...
		await Synthetix.anySynthOrSNXRateIsInvalid(blockOptions)
	);
	...
	if (useOvm) {
		...
		logItem(
			...
			(await SupplySchedule.mintBuffer(blockOptions)).toString()
		);
		logItem(
	...
	const logRate = async currency => {
		...
		const updated = await ExchangeRates.lastRateUpdateTimes(
