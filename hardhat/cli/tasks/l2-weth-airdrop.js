...
async function airdropWETH({
	...
	console.log(
		chalk.cyan(`1. Checking WETH balances on ${accounts.length} potential target accounts...`)
	);
	for (let i = 0; i < accounts.length; i++) {
		...
		console.log(
			chalk.gray(
				`  * Checking account ${i + 1}/${accounts.length} - ${account} - ${ethers.utils.formatEther(
					accountBalance
				)} WETH`
			)
		);

		const delta = amountToDrop.sub(accountBalance);
		if (delta.lte(zero)) {
	...
	if (signerBalance.lt(totalWethToDrop)) {
		throw new Error(
			`Signer only has ${ethers.utils.formatEther(
				signerBalance
			)} WETH, and it needs ${ethers.utils.formatEther(totalWethToDrop)} WETH.`
		);
	}
	...
	console.log(
		chalk.yellow('* target WETH weth balance for each:', ethers.utils.formatEther(amountToDrop))
	);
	console.log(
		chalk.yellow('* total WETH to be dropped:', ethers.utils.formatEther(totalWethToDrop))
	);
...
program
	...
	.option(
		'--data-file <value>',
		'The json file where target accounts are stored (generated by get-all-l2-active-snx-holders.js)'
	)
	.option(
		'--amount-to-drop <value>',
		'The total amount of WETH to be airdroped to each account',
		'0.05'
	)
