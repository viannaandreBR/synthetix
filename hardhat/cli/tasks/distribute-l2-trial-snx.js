...
async function distributeSNX({
	...
	if (!totalRewards)
		throw new Error('Please specify the total amount of SNX rewards to be distributed');
	...
	const rewardMultiplier = k
		.mul(totalRewardsWei)
		.div(ethers.BigNumber.from(data.totals.totalEscrowedSNX));
	...
	const pendingToDistribute = ethers.utils
		.parseEther(totalRewards)
		.sub(ethers.BigNumber.from(data.totals.distributedSNX));
	const signerBalance = await Synthetix.balanceOf(wallet.address);
	if (!dryRun) {
		if (signerBalance.lt(pendingToDistribute)) {
			throw new Error(
				red(
					`${pendingToDistribute} SNX need to be distributed and the signer only has ${signerBalance}`
				)
			);
		}
	}
	...
	console.log(
		yellow('* SNX already distributed:', ethers.utils.formatEther(data.totals.distributedSNX))
	);
	console.log(
		yellow('* SNX pending to be distributed:', ethers.utils.formatEther(pendingToDistribute))
	);
	...
	for (let i = 0; i < numAccounts; i++) {
		...
		if (balance.toString() !== '0' && distributed === '0') {
			console.log(
				yellow(`WARNING: account has a positive balance and has not been distributed to yet!`)
			);
			await confirm();
		}

		if (distributed === '0' && escrowed !== '0' && balance.toString() === '0') {
			const rewardAmount = ethers.BigNumber.from(data.accounts[account].escrowedSNX)
			...
			data.totals.distributedSNX = ethers.BigNumber.from(data.totals.distributedSNX)
				.add(rewardAmount)
				.toString();
